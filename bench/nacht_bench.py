...
# Korpus fuer die Langkontext-Dokumente: beliebige *.txt (z. B. Handbuecher). KORPUS_DIR setzen.
KORPUS = sorted(Path(os.environ.get("KORPUS_DIR", str(HIER.parents[1] / "sources" / "text"))).glob("*.txt"))
...
DOKCACHE = Path(os.environ.get("DOKCACHE", str(HIER / "dokumente_cache.json")))
